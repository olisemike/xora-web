    ...
    const loadFollowers = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_URL}/users/${encodeURIComponent(targetUsername)}/followers`,
          {
            method: 'GET',
            credentials: 'include',
            headers: {
        ...
        if (!res.ok || !data?.success) {
          throw new Error(
            data?.error?.message || `Failed to load followers (${res.status})`
          );
        }
        ...
        setFollowers(mapped);
        setError('');
      } catch (err) {setError(err.message || 'Failed to load followers');
        setFollowers([]);
      } finally {
        setLoading(false);
      }
